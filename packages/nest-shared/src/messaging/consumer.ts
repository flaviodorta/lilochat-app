...
export async function bindConsumer<TSchema extends z.ZodTypeAny>(
  ...
  // the whole topology registers as a ChannelSetup so a broker reconnect
  // re-asserts and re-consumes on the fresh channel automatically
  await bus.registerSetup(async (channel) => {
    ...
    await channel.consume(queue, (message) => {
      ...
      void (async () => {
        ...
        // infra hiccup outside the retry loop (e.g. Redis down during the
        // idempotency claim): pause, requeue, survive — never crash the process
        logger.warn({ err: error, queue }, 'consumer infra failure — requeueing');
        await sleep(1_000); // don't hot-spin the redelivery
        try {
          channel.nack(message, false, true);
        } catch {
          // channel died too — the reconnect loop redelivers unacked messages
        }
      });
    });
  });
